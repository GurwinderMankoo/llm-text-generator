              ...
              <div className="space-y-4">
                {sec.links.map((link) => (
                  <LinkFields
                    ...
                    onUrlChange={(val) =>
                      handleSectionHeadingChange({ secId: sec.id, linkId: link.id, value: val, type: "LINK", field: "url" })
                    }
                    onLabelChange={(val) =>
                      handleSectionHeadingChange({ secId: sec.id, linkId: link.id, value: val, type: "LINK", field: "label" })
                    }
                    onDescriptionChange={(val) =>
                      handleSectionHeadingChange({ secId: sec.id, linkId: link.id, value: val, type: "LINK", field: "description" })
                    }
                    onRemove={() =>
                      dispatch({ type: 'ADD_REMOVE_LINK_SECTION', payload: { type: 'REMOVE', secId: sec.id, linkId: link.id } })
                    }
                  />
                ))}
              </div>
        ...
        <div className="space-y-4">
          {optionalLinks.map((link) => (
            <LinkFields
              ...
              description={link.description}
              onUrlChange={(val) =>
                handleSectionHeadingChange({ linkId: link.id, value: val, type: "OPTIONAL_LINK", field: "url" })
              }
              onLabelChange={(val) =>
                handleSectionHeadingChange({ linkId: link.id, value: val, type: "OPTIONAL_LINK", field: "label" })
              }
              onDescriptionChange={(val) =>
                handleSectionHeadingChange({ linkId: link.id, value: val, type: "OPTIONAL_LINK", field: "description" })
              }
              onRemove={() =>
                dispatch({ type: 'ADD_REMOVE_OPTIONAL_LINK', payload: { type: 'REMOVE', linkId: link.id } })
              }
              removeTitle="Remove optional link"
            />
          ))}
        </div>
