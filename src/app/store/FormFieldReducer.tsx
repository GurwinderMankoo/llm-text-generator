...
export type FormFieldAction =
  ...
  | { type: "HANDLE_SECTION"; payload: { type: "HEADING" | "LINK" | "OPTIONAL_LINK"; secId?: string; linkId?: string; value: string, field?: 'url' | 'label' } }
  ...
  | { type: "ADD_REMOVE_OPTIONAL_LINK"; payload: { type: "ADD" | "REMOVE"; linkId?: string } }
    ...
    const onSectionChange = (
        state: FormFieldStateType,
        payload: { type: "HEADING" | "LINK" | "OPTIONAL_LINK"; secId?: string; linkId?: string; value: string, field?: 'url' | 'label' }
    ) => {
        const copiedState = structuredClone(state) as FormFieldStateType;
        ...
        if (type === "OPTIONAL_LINK" && linkId) {
            ...
            const field = payload.field as "url" | "label";
    ...
    const addRemoveSection = (
        ...
        if(payload.type === "ADD") {
            ...
            const newSection: SectionItem = {
                id: newId,
                heading: "",
                links: [{ id: `link-${newId}-0`, url: "", label: "" }]
            };
            ...
            newSection = state.sections.map(s => {
                if(s.id === payload.secId) {
                    return {
                        ...
                        links: [
                            ...s.links,
                            { id: `link-${payload.secId}-${Date.now()}`, url: "", label: "" }
                        ]
                    }
                }
                return s
